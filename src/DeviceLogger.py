    ...
    def get_logger(ip_address, output_dir="logs", console_level=None, format=None):
        ...
        with DeviceLogger._lock:
            if ip_address not in DeviceLogger._loggers:
                ...
                DeviceLogger._setup_device_logger(ip_address, full_output_dir, console_level, format)
        return DeviceLogger._loggers[ip_address]

    @staticmethod
    def _setup_device_logger(ip_address, output_dir, console_level, log_format):
        """
        ...
        file_path = output_dir / f"{output_dir}/device_monitor_debug_{ip_address}_{timestamp}.log"
        logger = logging.getLogger(f"device_{ip_address}")
        logger.setLevel(logging.DEBUG)
        file_handler = logging.FileHandler(str(file_path))  # Convert Path object to string for FileHandler
